...
export function setTargetedHostileFactory(factory, notOwnedFactories) {
    const { factory: nearestHostile, distance: distanceToNearestHostile } =
        notOwnedFactories.reduce(
            (result, hostileFactory) => {
                const isProductive =
                    result.production < hostileFactory.production
                const isNearer =
                    result.distance > hostileFactory.links[factory.id]
...
function enactBombCommand(notMine, myFactories, turn) {
...
function buildOrders(turn) {
    ...
    let bombCmd = enactBombCommand(notMine, myFactories, turn)
